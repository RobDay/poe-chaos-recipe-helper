import { ItemCategory, StashItem, ItemCategoryKeys, ItemType } from "../shared/models";
...
export default class RecipeManager {
  ...
  getChaosRecipes() {
    ...
    log.verbose("Generating chaos recipes");
    ...
    log.verbose("total chaos items: " + totalChaosItems);

    log.verbose("Regal Items");
    ...
    while (true) {
      ...
      if (RecipeManager.isRecipeSetComplete(recipeSet)) {
        log.verbose("Have a complete recipe set. Will replace an item");
        ...
        log.verbose("Have an incomplete recipe set. Will replace an item");
  ...
  _fillMissingWithChaosItems(partialRecipeSet: RecipeSet, chaosItems: GroupedStashItems): RecipeSet {
    ...
    for (let deterministicProperty of deterministicProperties) {
      if (!chaosItems[this._propertyNameToItemCategory(deterministicProperty)!]) {
        log.info(
